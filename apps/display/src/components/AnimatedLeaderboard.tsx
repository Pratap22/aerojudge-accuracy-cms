    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex-1 space-y-2">
        {rows.map((entry, index) => {
          const isHighlighted = entry.pilotNumber === highlightPilotNumber;
          return (
            <motion.div
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                {entry.countryCode2 && (
                  <p className="text-sm uppercase tracking-wider text-sky-400/70">{entry.countryCode2}</p>
                )}
              </div>
              <div className="text-right">
                ...
                {entry.bullseyes != null && entry.bullseyes > 0 && (
                  <p className="text-sm text-emerald-400">{entry.bullseyes} bullseye{entry.bullseyes !== 1 ? 's' : ''}</p>
                )}
              </div>
            </motion.div>
          );
